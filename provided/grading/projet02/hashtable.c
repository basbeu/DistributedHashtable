...
error_code add_Htable_value(Htable_t table, pps_key_t key, pps_value_t value)
{
    ...
    if(copied_value == NULL || copied_key == NULL) {
        //correcteur: dans le cas ou un strdup a réussi, il faut libérer sa mémoire avant de quitter (-0.5)
        return ERR_NOMEM;
    }
    ...
    while(b_temp != NULL && b_temp->pair.key != NULL) {
        ...
        if(b_temp->next != NULL) {
            ...
            if(insert == NULL) {
                //correcteur: il faut libérer la mémoire allouée avant de retourner l'erreur
                return ERR_NOMEM;
            }
            insert->next = NULL;
...
error_code kv_list_add(kv_list_t *list, kv_pair_t pair)
{
    ...
    //correcteur: il faut faire une copie de la paire (-0.5)
...
void kv_list_free(kv_list_t *list)
{
    if(list != NULL && list->list_pair != NULL) {
        //correcteur: il faut free chaque paire
        free(list->list_pair);
        list->list_pair = NULL;
        free(list);
        list =NULL;
