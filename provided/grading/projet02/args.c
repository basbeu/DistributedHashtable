...
int parse_arg(char ***argv, const char * arg_tag, size_t* arg_value)
{
    if(!strncmp((*argv)[0], "--", 2)) {
        ...
        int j = sscanf((*argv)[1],"%zu", arg_value); //correcteur: il ne faut pas lire avec %zu, sinon cast des nombres négatifs en positif (-0.5)
        (*argv)+=2;
...
args_t *parse_opt_args(size_t supported_args, char ***rem_argv)
{
    ...
    if(args != NULL) {
        //correcteur: il faut faire une boucle sur rem_argv tant qu'il y a des args où que -- n'a pas été trouvé (-1)
        int parsing_state = 0;
    ...
    //correcteur: il faut tester que les valeurs de N, W et R soient cohérentes (-1)
    return args;
}
