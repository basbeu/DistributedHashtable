...
int main(void)
{
    ...
    while(1) {
        ...
        if(in_msg_len != -1) {
            ...
            //dump request
            //correcteur: "\0" != '\0' ~ "" et il faut aussi tester que in_msg_len == 1 (-0.5)
            if(strncmp(in_msg, "/0", 1) == 0) {
                ...
                if(list_of_pairs != NULL) {
                    ...
                    //correcteur: %zu écrit un size_t sur 8 bytes alors qu'il est demandé d'écrire la taille sur 4 bytes (-2)
                    //correcteur: et il faudrait utiliser htonl
                    sprintf(out_msg, "%zu", list_of_pairs->size);
                    out_msg_len += 4;

                    //correcteur: i est déjà défini à la ligne 51 (-0.5) il faudrait activer les warnings lors de la compilation afin d'éviter ce genre d'erreur
