...
error_code add_Htable_value(Htable_t table, pps_key_t key, pps_value_t value)
{
    ...
    char* val = calloc(value_len, sizeof(char));            // correcteur astuce: strdup pour une copie facile de "string" (voir la manpage)
    strncpy(val, value, value_len);
    pps_value_t copied_value = val;
    char* k = calloc(key_len, sizeof(char));                // correcteur: vous ne vérifiez pas la valeur de retour de calloc + vous copiez-collez ce code (-1pts)
    ...
    while(b_temp != NULL && b_temp->pair.key != NULL) {
        if(strncmp(b_temp->pair.key, key, MAX_MSG_ELEM_SIZE) == 0) {
            b_temp->pair.key = copied_key;                                  // correcteur: vous ne libérez pas l'ancienne kv_pair (-0.5pts)
        ...
        if(b_temp->next != NULL) {
            ...
            bucket_t* insert = calloc(1, sizeof(bucket_t));                 // correcteur: vous ne vérifiez pas la valeur de retour de calloc (-0.5pts)
            insert->next = NULL;
            kv_pair_t pair;
            pair.key = copied_key;
            pair.value = copied_value;
            insert->pair = pair;                                            // correcteur astuce: vous pouvez utiliser la notation: insert->pair = {copied_key, copied_value} (vu en cours)
            b_temp->next = insert;
            return ERR_NONE;
        }
    }
    if(b_temp != NULL) {                                                    // correcteur: dans quel cas execute-t-on ce if ??
        b_temp->pair.key = copied_key;                                      // correcteur: vous ne libérez pas l'ancienne kv_pair (-0.5pts)
        b_temp->pair.value = copied_value;
    }
...
pps_value_t get_Htable_value(const Htable_t table, pps_key_t key)
{
    ...
    while(b->next != NULL) {
        if(strncmp(b->pair.key, key, MAX_MSG_ELEM_SIZE) == 0) {
            return b->pair.value;                                       // correcteur: vous devez retourner une COPIE de la valeur (-0.5pts)
        }
        b = b->next;
    }

    if(b->pair.key != NULL && strncmp(b->pair.key, key, MAX_MSG_ELEM_SIZE) == 0) {
        return b->pair.value;                                           // correcteur: vous devez retourner une COPIE de la valeur
    } else {
        return NULL;
    }
...
Htable_t construct_Htable(size_t size)
{
    ...
    if(size > 0) {
        ...
        if(table.bucket != NULL) {
            table.size = size;
            for(size_t i = 0; i < size; ++i) {          // correcteur astuce: quand vous utilisez calloc, toute la mémoire est initialisée à 0 (et donc NULL) => pas besoin de cette boucle
                kv_pair_t pair;
                pair.key = NULL;
                pair.value = NULL;
...
{                                                   // correcteur: vous devriez aussi tester si kv est NULL
    if(kv->key != NULL) {
...
void bucket_free(bucket_t* bucket)
{
    if(bucket != NULL) {
        ...
        while(b_next != NULL) {                     // correcteur: vous ne libérez pas la mémoire de la kv_pair de chaque bucket (-0.5pts)
            b_curr = b_next;
            b_next = b_curr->next;
            free(b_curr);
            b_curr = NULL;
