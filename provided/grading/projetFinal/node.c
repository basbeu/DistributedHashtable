...
error_code node_init(node_t *node, const char *ip, uint16_t port, size_t _unused node_id)
{
    ...
    if(strncpy(sha, ip, strlen(ip)) != NULL) {
        ...
        err = ERR_NOMEM;                    // correcteur: dans ce cas vous oubliez de libérer node->ip (-0.5pts)
    }
    return err;
}
...
{
    return strncmp((const char*)first->sha, (const char*)second->sha, SHA_DIGEST_LENGTH);                    // correcteur: vous ne pouvez pas utiliser strncmp ici, un hash peut contenir des 0 (-0.5pts)
}
